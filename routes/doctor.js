...
router.get("/list",
    [
        ...
        query("sortOrder").optional().isIn(["ac", "desc"]),
...
router.get("/me", authenticate, requireRole("doctor"), async (req, res, next) => {
...
router.put("/onboarding/update", authenticate, requireRole("doctor"),
  [
    ...
    body("availabilityRange.excludesWeakDays").optional().isArray(),
    body("dailyTimeRanges").isArray({ min: 1 }),
    body("dailyTimeRanges.*.start").isString(),
    body("dailyTimeRanges.*.end").isString(),
    body("slotDurationMinutes").optional().isInt({ min: 5, max: 100 }),
